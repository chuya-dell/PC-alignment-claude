...
def _subpixel_argmin(arr, axis=-1):
    """最小値位置を放物線補間でサブピクセル精度に求める（端の場合は補間なし）"""
    arr = np.asarray(arr, dtype=np.float32)
    idx = np.argmin(arr, axis=axis)
    idx_c = np.clip(idx, 1, arr.shape[axis] - 2)
    y0 = np.take_along_axis(arr, np.expand_dims(idx_c - 1, axis), axis=axis).squeeze(axis)
    y1 = np.take_along_axis(arr, np.expand_dims(idx_c, axis), axis=axis).squeeze(axis)
    y2 = np.take_along_axis(arr, np.expand_dims(idx_c + 1, axis), axis=axis).squeeze(axis)
    denom = y0 - 2 * y1 + y2
    offset = np.where(denom != 0, 0.5 * (y0 - y2) / np.where(denom == 0, 1, denom), 0.0)
    at_edge = (idx == 0) | (idx == arr.shape[axis] - 1)
    offset = np.where(at_edge, 0.0, offset)
    return idx.astype(np.float64) + offset

def detect_scratch_angle(img):
    """縦線の傾き（度）を各行の最暗点（サブピクセル）の線形回帰で検出"""
    c0, c1 = SCRATCH_COL_RANGE
    strip = img[:, c0:c1].astype(np.float32)
    dark_cols = _subpixel_argmin(strip, axis=1) + c0
...
def detect_scratch_col(img, col_search=(30, 300)):
    """縦線colをサブピクセル精度で検出（端の暗い領域を除いた範囲で最小値）"""
    arr = img.astype(np.float32)
    c0, c1 = col_search
    col_means = arr[:, c0:c1].mean(axis=0)
    return float(_subpixel_argmin(col_means)) + c0
...
def detect_scratch_row(img, row_search=(1800, 2044), col_search=(200, 2048)):
    ...
    return float(_subpixel_argmin(row_means)) + r0
...
for name in TARGETS:
    ...
    col_margin = 150
    col_lo = max(30, int(tgt_col) - col_margin)
    col_hi = min(2040, int(tgt_col) + col_margin)
    ...
    print(f"  平行移動: dx={dx:+.2f}px  dy={dy:+.2f}px  |shift|={total:.2f}px")
...
for r in results:
    print(f"{r['file']:<12} {r['rot']:>+10.4f} {r['dx']:>+8.2f} {r['dy']:>+8.2f} {r['total']:>12.2f}")

...
→(**`_trace_line`が"position"を単独確定する構造をやめ、band内ローカル座標の
  ...
  実データでの再検証はまだ行っていない段階だった。
→(**ユーザーPCで `F:\GoogleDrive_local\1.実験データ_gdrive\TEST`(8ペア、
  「4.生データ D」とは別の小規模データセット)を用いて実データ再検証**)→
  **8/8検出成功、scratch_residual_px median 0.62px・max 2.7px** に大幅改善
  (旧構造62710bd時点は median 3.61〜3.7px)。全視野が`registration_mode`=
  `scratch_translation`(ECC残差の方が悪くフォールバックが採用された形)。
  残差>2pxは1視野のみ(condition1-set5, residual 2.695px, confidence0.956と
  高信頼度)で、以前観測された数十〜百px級の暴走とは性質が異なる小さな超過。
  **ただしTESTはn=8と小規模**なため、「4.生データ D」(n=64)での確認と
  condition1-set5の個別診断が未着手。

## 到達点(2026-07-15時点、更新中)
scratch_residual_px median は 21.21→6.4→5.0→3.61→3.7px(62710bd)→**0.62px(cb30363、
TEST n=8で確認)**と推移。5つの実バグ(極性軸混同/境界黒縁誤検出/pre-post極性
不一致/傾き非対応モード投票/交点位置の基準行不一致)を発見・修正・合成テストで
再発防止済み(cb30363で交点ベース位置推定に構造変更)。小規模データセットTESTでの
実データ検証では効果を確認できたが、**「4.生データ D」(n=64、実データ物理パラメータの
確定元データ)でのフル再検証はまだ**。過去にも合成→小規模実データでは良好でも
フルデータで新たな回帰が出た前例(cbd846b→7-4回帰等)があるため、n=64での確認が
必要な段階。

## 次アクション(NEXT)
1. **[ユーザーPC] condition1-set5(TEST, residual2.695px)を`diagnose_scratch.py
   --image2`で個別診断**。高信頼度(0.956)なのに2px閾値を僅かに超えている原因を
   確認(小さな残存バグか、正常なノイズ範囲かの切り分け)。
2. **[ユーザーPCでフル実データ再検証]** commit cb30363の効果を
   `F:\GoogleDrive_local\1.実験データ_gdrive\4.生データ\4.生データ D\260704 sam 位置合わせ test\df`
   (n=64)でも確認。`git pull` → 実行方法セクション参照 → 品質チェックスクリプトで
   scratch_residual_px median/maxと残差>2pxの視野数を確認。悪化した視野があれば
   `diagnose_scratch.py --image2` で個別診断。
3. **[ユーザー] pixel pitch ≈ 32 nm/px を顕微鏡キャリブレーションで確認。**
4. **[ユーザー] -anti の残ブランチ `claude/batch-image-registration-qe4cp7` を削除**
   (私はegressポリシー403で削除不可。GitHub UIのBranchesから)。
5. **[次の主要ゴール] 差分画像生成 → Grid_Heatmapマクロへの受け渡し形式。**

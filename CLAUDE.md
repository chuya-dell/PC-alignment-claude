...
→(_trace_line にサブピクセル補間を追加: commit a2de478)→ **実データ再実行で判明**:
  ...
  サブピクセル化は害がほぼないため維持。
→(**最悪ケース2-3を診断: 完了、新バグなしと結論**)← 最新。confidence=0.37・
  波打ちstd=12.7pxという2-3を可視化 → 縦の傷帯はpre/postとも同じ物理位置に一貫して
  存在するが、**帯自体の明るさが場所によってムラがあり(部分的に途切れる)**、これが
  不安定な検出の直接原因と判明。これまでの3バグのような明確な指紋(極性食い違い・
  境界黒縁誤検出・pre/post別特徴追跡)は見当たらず。**結論: これ以上の明確なコード
  バグは無く、残る残差(median≈3.6-3.9px, max≈22px)は信号品質そのもののムラ・
  ノイズフロアに起因すると判断。バグ探索フェーズはここで区切りとする。**

## 到達点(2026-07-14時点)
4段階の実データ検証サイクルで scratch_residual_px median を 21.21→6.4→5.0→3.61→3.89px
まで改善し、3つの実バグ(極性軸混同/境界黒縁誤検出/pre-post極性不一致)を発見・修正・
合成テストで再発防止済み。残る残差はconfidence 0.35〜0.6程度の低〜中信頼度視野に偏り、
信号品質(コントラストムラ・波打ち)由来のノイズフロアに到達したと判断する。
既定パラメータ(min-confidence 0.35, trace-tol 20, min-contrast 2.0, crop-margin 250,
residual-tol 2.0)を運用値として確定する。

## 次アクション(NEXT)
1. **[ユーザー] 現状を受け入れて運用に進むか、confidence閾値等の方針をさらに詰めるか判断。**
   - 低confidence(<0.5程度)の視野は残差が数px〜20px程度残り得ることを踏まえ、下流解析で
     `scratch_confidence_pre`/`scratch_residual_px` を参照して個別に信頼度判断する運用を推奨。
   - さらに追い込みたい場合は残る視野を追加診断する余地はあるが収穫逓減が見込まれる。

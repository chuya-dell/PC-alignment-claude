...
→(**positionをband中心行への外挿から、2nd passで得た正しいインライア集合の中央値へ
  戻して修正: 本コミット 62710bd**)← 最新。デトレンドによる正しいインライア発見
  (傾き対応の本質)は維持しつつ、外挿でなく内挿で完結させた。7-4のコーナー屈曲を模した
  合成シーンで、旧(N/2外挿)residual=4.76px→新(中央値)residual=0.71pxに改善することを
  確認(実データの51pxほどの規模は合成では未再現だが方向は一致)。既存の全回帰テスト
  (TEST1〜8)PASS。**実データでの再検証がまだ済んでいない**(要ユーザー再実行)。

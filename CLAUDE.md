...
→(**`_trace_line`にサブピクセル補間を追加: 本コミット a2de478**)← 最新。合成データで
  全テスト(TEST1〜7)の scratch_residual_px が軒並み1px未満(0.06〜0.81px)に改善する
  ことを検証、全テストPASS。**実データでの再検証がまだ済んでいない**(要ユーザー再実行)。
